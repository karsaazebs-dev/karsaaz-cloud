...
} from "@/lib/hooks/useExternalStorage";
...
const BACKEND_CONFIG: Record<ExternalStorageBackend, BackendConfig> = {
  ...
  local: { authMechanism: "null::null", fields: ["datadir"] },
...
const HOST_PLACEHOLDER: Partial<Record<ExternalStorageBackend, string>> = {
...
export default function AdminExternalStoragePage() {
  const { data: storages, isLoading } = useExternalStorages();
  const createStorage = useCreateExternalStorage();
  const deleteStorage = useDeleteExternalStorage();

  const [backend, setBackend] = useState<ExternalStorageBackend>("dav");
  ...
  const primaryValue =
    backend === "local" ? datadir : backend === "amazons3" ? bucket : host;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Connect external storage available to all users.</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configured global external storages.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          {isLoading ? (
            <Skeleton className="h-12 w-full" />
          ) : !storages || storages.length === 0 ? (
            <p className="text-sm text-muted-foreground">No external storage configured.</p>
          ) : (
            storages.map((s) => (
              <div key={s.id} className="flex items-center gap-3 rounded-md border px-3 py-2 text-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    ...
                    {typeof s.backendOptions?.datadir === "string" && ` · ${s.backendOptions.datadir}`}
                  </p>
                </div>
                <Button
                  size="icon"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Mount an external storage backend for all users.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label>Type</Label>
              <Select value={backend} onValueChange={(v) => setBackend(v as ExternalStorageBackend)}>
            ...
            {has("datadir") && (
              <div className="space-y-1.5">
                <Label htmlFor="es-datadir">Location</Label>
                <Input id="es-datadir" value={datadir} onChange={(e) => setDatadir(e.target.value)} placeholder="/path/on/server" />
              </div>
            )}
