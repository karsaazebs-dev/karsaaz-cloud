...
class Version003Date20260618120000 extends SimpleMigrationStep {
    public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
        /** @var ISchemaWrapper $schema */
        $schema = $schemaClosure();

        if ($schema->hasTable('kz_quota_alloc')) {
            $table = $schema->getTable('kz_quota_alloc');
            ...
            if (!$table->hasColumn('profile_json')) {
                $table->addColumn('profile_json', Types::TEXT, [
                    'notnull' => false,
                ]);
            }
        }
        ...
        if ($schema->hasTable('kz_quota_requests')) {
            ...
            if (!$table->hasColumn('admin_notes')) {
                $table->addColumn('admin_notes', Types::TEXT, [
                    'notnull' => false,
                ]);
            }
        }
